...
echo $this->Form->create('ExerciseCase'); ?>
    ...
    <div class="row-fluid">
        <div class="exercises index widget span12">
            ...
            <div class="widget-body">
                <fieldset>
                    <legend><?php echo __('Edit Exercise Case'); ?></legend>
                    ...
                    echo $this->Form->input('input', array('label' => array('text' => __('Input'),'class' => 'control-label'), 'type' => 'textarea','class' => 'span4','div' => array('class' => 'control-group'))); 
                    echo $this->Form->input('maxmemsize', array('label' => array('text' => __('Max. Memory Size'),'class' => 'control-label'), 'type' => 'select','options'=>$memsizeoptions,'class' => 'span4','div' => array('class' => 'control-group'))); 
                    echo $this->Form->input('stacksize', array('label' => array('text' => __('Max. Stack Size'),'class' => 'control-label'), 'type' => 'select','options'=>$stacksizeoptions,'class' => 'span4','div' => array('class' => 'control-group'))); 
                    echo $this->Form->input('file_size', array('label' => array('text' => __('Max. Blocks Use'),'class' => 'control-label'), 'type' => 'select','options'=>$blocksizeoptions,'class' => 'span4','div' => array('class' => 'control-group'))); 
                    echo $this->Form->input('cputime', array('label' => array('text' => __('Max. CPU Time'),'class' => 'control-label'), 'type' => 'select','options'=>$timeoptions,'class' => 'span4','div' => array('class' => 'control-group'))); 
                    ?>
            ...
            <div class="widget-body">
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane" id="output-numbers">
                        <p class="text-info"><?php echo __("For the numeric output the system will compare the student output numbers with the case output numbers considering the absolute error allowed. You must inform the maximum absolute error allowed for the output numbers"); ?></p>
        ...
        <div class="exercises index widget span12">
            ...
            <div class="widget-body exercise-files-inputs">
                <p>
                    ...
                    <br><?php echo __("Do not forget to save the changes after upload any new files above"); ?>
                    <br><?php if(count($files) > 0) echo __("A click in %s will erase the selected file instantly", __("Remove")); ?>
                </p>
                <div class="exercise-files-list">
                    <?php 
                    foreach ($files as $key => $file) : ?>
                    <div class="exercise-file-item" id="file<?php echo $file['id']; ?>">
                        <?php echo $this->Html->link(__('Remove'), '#',array('escape' => false,'data-file-id' => $file['id'],'class' => 'btn btn-small btn-danger existent-exercise-file')); ?>
                        <?php    echo $file['path']."<br><input type='hidden' name='data[ExerciseCaseFile][" . $key . "][path]' value='" . $file['path'] . "' /><input type='hidden' name='data[ExerciseCaseFile][" . $key . "][hash]' value='0' /><input type='hidden' name='data[ExerciseCaseFile][" . $key . "][id]' value='". $file['id'] ."' />"; ?>
                    </div>
                    <?php endforeach; ?>
                </div>
...
<script>
$(function () {
    var count_files=<?php echo count($files); ?>;
    ...
    $(".existent-exercise-file").on("click", function(e) {
        e.preventDefault();
        var file = $(this).attr('data-file-id');
        $.post('/ExerciseCaseFiles/delete', {dataExerciseFileCaseId: file }, function (ret) {
            if (ret == "1") {
                $("#file" + file).remove();
            } else {
                alert("<?php echo __("Sorry! Error"); ?>: " + ret);
            }
        });
    });
